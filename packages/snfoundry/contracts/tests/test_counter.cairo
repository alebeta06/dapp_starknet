...
fn test_set_counter_owner() {
    ...
    assert!(
        dispatcher.get_counter() == new_counter,
        "The owner is unable to reset the counter"
    );
...
fn test_reset_counter_success() {
    ...
    assert!(
        ...
        "Unable to reset counter even with enough STRK"
    );
    ...
    assert!(
        erc_20.balance_of(user) == strk_to_fri(9),
        "Balance not deducted correctly"
    );
    assert!(
        erc_20.balance_of(owner_address()) == strk_to_fri(1),
...
#[test]
#[should_panic(expected: "Contract is not allowed to spend the user stark tokens")]
